...
STQueue *createSTQueueFromRandomValues(int numRequestsToGenerate, int numTestsToRun) {
    // This function runs numTestsToRun to test the ST queue. Each test consists of one
        ...
        if (numRequestsGenerated < numRequestsToGenerate && actionRange >= ADD_A_REQUEST && actionRange < GET_A_REQUEST) {
            ...
            if (queue->empty())
                ...
                if(request->time() < previousTimeStamp)
                    std::cout << "Error: The STQueue just returned a request whose timestamp is out of order.\n";
