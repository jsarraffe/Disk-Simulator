...
CLookQueue *createCLookQueueFromRandomValues(int numRequestsToGenerate, int numTestsToRun) {
    // This function runs numTestsToRun to test the CLook queue. Each test consists of one
    ...
    // CLook is a simple queue and as a result, testing whether it organizes the requests
    ...
    while ( numTestsRun < numTestsToRun ) {
        ...
        if (numRequestsGenerated < numRequestsToGenerate && actionRange >= ADD_A_REQUEST && actionRange < GET_A_REQUEST) {
            ...
            request = generateRandomRequest();
            std::cout << "Adding the following request to the CLookQueue.\n";
            request->print();
            ...
            if (queue->empty())
                std::cout << "CLookQueue is empty.\n";
            else {
                request = queue->getRequest();
                std::cout << "Removed the following request from the CLookQueue via getRequest.\n";
                request->print();
                if(request->time() < previousTimeStamp)
                    std::cout << "Error: The CLookQueue just returned a request whose timestamp is out of order.\n";
            ...
            if (numRequestsInTheQueue != 0 && queue->empty()) {
                std::cout << "CLookQueue::empty() returns true, but the queue should have " <<
                          numRequestsInTheQueue << " elements in it." << std::endl;
                exit(1);
            } else if (numRequestsInTheQueue == 0 && !queue->empty()) {
                std::cout << "CLookQueue::empty() returns false, but the queue should be empty." << std::endl;
                exit(1);
            }
        }
        numTestsRun++;
    ...
    while(! queue->empty() ) {
        std::cout << "Removed the following request from the CLookQueue after having tested the queue.\n";
    ...
    if( numRequestsInTheQueue > 0 ) {
        std::cout << "In the process of testing, the CLookQueue lost " << numRequestsInTheQueue <<
                  ". This means the queue contains a bug!\n";
    } else if(numRequestsInTheQueue < 0) {
        std::cout << "In the process of testing, the CLookQueue did not remove " << -numRequestsInTheQueue <<
...
int main(int argc, char *argv[]) {
    ...
    if( randomTest )
        queue = createCLookQueueFromRandomValues(300, 200);
    else
    ...
    CLookQueue *queue2 = new CLookQueue();
